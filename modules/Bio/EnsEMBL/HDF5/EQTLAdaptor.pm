...
sub new {
  ...
  if (! -e $hdf5_file || -z $hdf5_file) {
    ...
    my $curated_snp_id_file = $hdf5_file.".snp.ids";
    if (! -e $curated_snp_id_file || -z $curated_snp_id_file) {
	    _curate_variant_names($variation_db, $snp_id_file, $curated_snp_id_file);
    }
